...
function rejectAllHelicopters()
	for i=1,#choppers do
		QueryRejectBody(choppers[i])
	end
	for i=1,#tailrotors do
		QueryRejectBody(tailrotors[i])
	end
	for i=1,#mainrotors do
		QueryRejectBody(mainrotors[i])
	end
	for i=1,#lights do
		QueryRejectBody(lights[i])
	end
end
